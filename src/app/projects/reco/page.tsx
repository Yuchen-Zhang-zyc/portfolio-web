...
const CSS = `
  .reco-page { font-family: var(--font-dm-sans, 'DM Sans', sans-serif); background: #ECEEF5; color: rgba(27,42,107,0.88); min-height: 100vh; }
  ...
  /* Section */
  .reco-section { padding: 96px 0; border-top: 1px solid rgba(27,42,107,0.1); }
  .reco-section-first { border-top: none; padding-top: 80px; }

  /* Typography */
  .reco-eyebrow { font-family: var(--font-dm-mono, 'DM Mono', monospace); font-size: 10px; letter-spacing: 0.14em; text-transform: uppercase; color: #2D4A9E; margin-bottom: 12px; }
  .reco-h1 { font-size: clamp(48px, 7vw, 80px); font-weight: 700; line-height: 1.0; letter-spacing: -0.03em; margin-bottom: 24px; color: #1B2A6B; }
  .reco-h2 { font-size: clamp(24px, 3vw, 36px); font-weight: 600; letter-spacing: -0.02em; line-height: 1.2; color: #1B2A6B; margin-bottom: 16px; }
  .reco-lead { font-size: 17px; color: rgba(27,42,107,0.72); line-height: 1.8; max-width: 600px; }

  /* Meta grid */
  .reco-meta { display: grid; grid-template-columns: repeat(5, 1fr); gap: 1px; background: rgba(27,42,107,0.1); border: 1px solid rgba(27,42,107,0.1); border-radius: 12px; overflow: hidden; margin-top: 56px; }
  .reco-meta-cell { padding: 20px; background: #FFFFFF; }
  .reco-meta-label { font-family: var(--font-dm-mono, 'DM Mono', monospace); font-size: 9px; text-transform: uppercase; letter-spacing: 0.12em; color: rgba(27,42,107,0.45); margin-bottom: 6px; }
  .reco-meta-value { font-size: 13px; color: rgba(27,42,107,0.85); font-weight: 500; }
  @media (max-width: 1024px) { .reco-meta { grid-template-columns: repeat(3, 1fr); } }
  @media (max-width: 640px) { .reco-meta { grid-template-columns: 1fr 1fr; } }

  /* Role highlight */
  .reco-role-badge { display: inline-flex; align-items: center; gap: 8px; background: rgba(91,140,245,0.12); border: 1px solid rgba(91,140,245,0.35); padding: 6px 14px; border-radius: 100px; font-size: 12px; color: #1B2A6B; font-family: var(--font-dm-mono, 'DM Mono', monospace); letter-spacing: 0.04em; margin-bottom: 24px; }
  ...
  /* Tech stack cards */
  .reco-stack { display: flex; flex-direction: column; gap: 1px; background: rgba(27,42,107,0.1); border: 1px solid rgba(27,42,107,0.1); border-radius: 12px; overflow: hidden; }
  .reco-stack-row { display: grid; grid-template-columns: 200px 1fr; background: #FFFFFF; }
  .reco-stack-label { padding: 16px 20px; font-family: var(--font-dm-mono, 'DM Mono', monospace); font-size: 11px; color: rgba(27,42,107,0.5); text-transform: uppercase; letter-spacing: 0.08em; border-right: 1px solid rgba(27,42,107,0.08); display: flex; align-items: center; }
  .reco-stack-value { padding: 16px 20px; font-size: 13px; color: rgba(27,42,107,0.78); line-height: 1.6; display: flex; align-items: center; gap: 10px; }
  .reco-stack-tag { font-family: var(--font-dm-mono, 'DM Mono', monospace); font-size: 10px; background: rgba(91,140,245,0.14); color: #2D4A9E; padding: 3px 8px; border-radius: 4px; white-space: nowrap; }
  @media (max-width: 640px) { .reco-stack-row { grid-template-columns: 1fr; } .reco-stack-label { border-right: none; border-bottom: 1px solid rgba(27,42,107,0.08); } }

  /* Contribution items */
  .reco-contrib-list { list-style: none; padding: 0; margin: 0; }
  .reco-contrib-item { border-top: 1px solid rgba(27,42,107,0.1); padding: 24px 0; display: flex; gap: 20px; }
  .reco-contrib-num { font-family: var(--font-dm-mono, 'DM Mono', monospace); font-size: 10px; color: #3B6FD9; flex-shrink: 0; padding-top: 4px; min-width: 24px; }
  .reco-contrib-title { font-size: 15px; font-weight: 600; color: #1B2A6B; margin-bottom: 6px; }
  .reco-contrib-body { font-size: 13px; color: rgba(27,42,107,0.65); line-height: 1.7; }

  /* Feature image */
  .reco-feature-img { width: 100%; border-radius: 12px; display: block; margin-top: 16px; border: 1px solid rgba(27,42,107,0.08); box-shadow: 0 8px 28px rgba(27,42,107,0.06); }

  /* Feature cards */
  .reco-features { display: grid; grid-template-columns: repeat(3, 1fr); gap: 1px; background: rgba(27,42,107,0.1); border: 1px solid rgba(27,42,107,0.1); border-radius: 12px; overflow: hidden; margin-top: 40px; }
  .reco-feature-card { background: #FFFFFF; padding: 28px 24px; }
  .reco-feature-icon { font-size: 22px; margin-bottom: 12px; }
  .reco-feature-name { font-size: 15px; font-weight: 600; color: #1B2A6B; margin-bottom: 8px; }
  .reco-feature-desc { font-size: 13px; color: rgba(27,42,107,0.62); line-height: 1.65; }
  @media (max-width: 700px) { .reco-features { grid-template-columns: 1fr; } }

  /* Stat row */
  .reco-stats { display: flex; gap: 1px; background: rgba(27,42,107,0.1); border: 1px solid rgba(27,42,107,0.1); border-radius: 12px; overflow: hidden; margin-top: 40px; flex-wrap: wrap; }
  .reco-stat { padding: 24px 28px; background: #FFFFFF; flex: 1; min-width: 120px; }
  .reco-stat-num { font-size: 32px; font-weight: 700; color: #1B2A6B; display: block; letter-spacing: -0.03em; }
  .reco-stat-label { font-family: var(--font-dm-mono, 'DM Mono', monospace); font-size: 10px; color: rgba(27,42,107,0.5); display: block; margin-top: 4px; text-transform: uppercase; letter-spacing: 0.08em; }

  /* Pullquote */
  .reco-pullquote { border-left: 2px solid #5B8CF5; padding-left: 24px; margin: 40px 0; font-size: 18px; font-weight: 400; font-style: italic; color: rgba(27,42,107,0.72); line-height: 1.65; }

  /* HiFi gallery */
  .reco-hifi-scroll { display: flex; gap: 16px; overflow-x: auto; padding-bottom: 16px; margin-top: 40px; scrollbar-width: thin; scrollbar-color: rgba(27,42,107,0.25) transparent; }
  .reco-hifi-scroll::-webkit-scrollbar { height: 4px; }
  .reco-hifi-scroll::-webkit-scrollbar-track { background: transparent; }
  .reco-hifi-scroll::-webkit-scrollbar-thumb { background: rgba(27,42,107,0.22); border-radius: 2px; }
  .reco-hifi-frame { flex-shrink: 0; width: 220px; border-radius: 28px; overflow: hidden; border: 1px solid rgba(27,42,107,0.12); background: #FFFFFF; box-shadow: 0 12px 36px rgba(27,42,107,0.1); }
...
export default function RecoPage() {
  return (
    <>
      ...
      <div className="reco-page">
        ...
        <section className="reco-section reco-section-first" id="main-content">
          <div className="reco-container">
            <div className="reco-eyebrow">48-Hour School Hackathon · iOS · Group Busy BEE · 2025</div>
            <h1 className="reco-h1">
              <span style={{ color: "#1B2A6B" }}>RECO</span>
            </h1>
            ...
            <p className="reco-lead" style={{ marginTop: 16, maxWidth: 640, fontSize: 15 }}>
              <strong style={{ color: "#1B2A6B", fontWeight: 600 }}>Context:</strong> This project was conceived and built in{" "}
              <strong style={{ color: "#1B2A6B", fontWeight: 600 }}>48 hours</strong> during an on-campus school hackathon. Our team, Group Busy BEE, received an{" "}
              <strong style={{ color: "#1B2A6B", fontWeight: 600 }}>Honorable Mention</strong> for the work.
            </p>
            ...
            <div className="reco-meta">
              {[
                ...
                { label: "Event", value: "48-hour school hackathon" },
                { label: "Recognition", value: "Honorable Mention" },
          ...
          <div className="reco-container">
            <div className="reco-grid-2">
              <div>
                ...
                <p style={{ fontSize: 15, color: "rgba(27,42,107,0.68)", lineHeight: 1.8, marginBottom: 20 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 12, color: "rgba(27,42,107,0.42)", marginTop: 12, lineHeight: 1.6, fontFamily: "var(--font-dm-mono,'DM Mono',monospace)" }}>
        ...
        <section className="reco-section" id="solution">
          <div className="reco-container">
            ...
            <p style={{ fontSize: 15, color: "rgba(27,42,107,0.68)", lineHeight: 1.8, maxWidth: 640, marginBottom: 40 }}>
        ...
        <section className="reco-section" id="hifi">
          <div className="reco-container">
            ...
            <p style={{ fontSize: 15, color: "rgba(27,42,107,0.68)", lineHeight: 1.8, maxWidth: 600 }}>
        ...
        <section className="reco-section" id="contribution">
          <div className="reco-container">
            ...
            <p style={{ fontSize: 15, color: "rgba(27,42,107,0.68)", lineHeight: 1.8, maxWidth: 600, marginBottom: 40 }}>
          ...
          <div className="reco-container">
            <div style={{ maxWidth: 680 }}>
              ...
              <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
                {[
                  { mark: "→", text: "A 48-hour school hackathon meant ruthless prioritization — we focused on a credible AI + PubMed pipeline and a demo-ready UI. Earning Honorable Mention validated that the jury saw a clear concept and solid execution under extreme time pressure." },
                  ...
                  <li key={i} style={{ borderTop: "1px solid rgba(27,42,107,0.1)", padding: "18px 0", display: "flex", gap: 16, fontSize: 14, color: "rgba(27,42,107,0.72)", lineHeight: 1.7 }}>
                    <span style={{ fontFamily: "var(--font-dm-mono,'DM Mono',monospace)", fontSize: 11, color: "#3B6FD9", flexShrink: 0, paddingTop: 2 }}>{item.mark}</span>
                    <span>{item.text}</span>
                  </li>
                ))}
              </ul>
      ...
      <NextProject href="/projects/this-website" title="This Website" role="Designer & Developer" theme="light" />
    </>
  );
}
